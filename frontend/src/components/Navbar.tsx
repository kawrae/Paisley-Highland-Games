  ...
  return (
    <header className="sticky top-0 z-40 border-b bg-white/70 backdrop-blur">
      <div className="container-page h-14 flex items-center justify-between">
        <Link to="/" className="text-highland-800 font-display font-extrabold text-lg tracking-tight">
        ...
        <nav className="flex items-center gap-1">
          ...
          {user ? (
            <div className="flex items-center gap-2 pl-2">
              {user.role === "admin" && (
                <span className="rounded-full bg-highland-100 text-highland-800 text-xs px-2 py-1">Admin</span>
              )}
              <button
                onClick={() => { logout(); nav("/"); }}
